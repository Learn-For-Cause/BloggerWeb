...
PostBody.getInitialProps = async ({ query }) => {
  const { id } = query;
  return { id };
}
...
export default function PostBody({ id }) {
  ...
  const dataPost = getPostDataById(id);
  ...
  const FetchPostData = () => {
    axios
      .get(`http://localhost:3000/api/bloglist/${id}`)
      .then((res) => {
        //alert(JSON.stringify(res.data.data._id));
          ...
          setPosts(posts => [
           {
            id:res.data.data._id,
             ...
             data: {
               name: res.data.data.blogsadd.blogWriter,
               title: res.data.data.blogsadd.blogName,
               description: res.data.data.blogsadd.blogDesc,
               paragraph: res.data.data.blogsadd.blogDesc,
               image: img, //change in later
               dateBlog:res.data.data.blogsadd.blogDate,
               timeBlog:res.data.data.blogsadd.blogTime,
               publication:res.data.data.blogsadd.publication,
             },
           },
         ])
      ...
      <div className={styles.postBody}>
        ...
        {loading ? (
          ...
          <div className={styles.postBody_container}>
            <div className={styles.postBody_header}>
              <div className={styles.postBody_name}>
                <Link href="#"><h2
                >{dataPost.name}</h2></Link>
            ...
            <hr></hr>
            <h1 style={{ textAlign: "center", margin: "2px 10% 0 10%" }}>{dataPost.title}</h1>
            <h2 style={{ textAlign: "center", color: "grey", margin: "2px 10% 5px 10%" }}>{dataPost.description}</h2>
            
            <div className={styles.postBody_image}>
              <img src={dataPost?.image} alt="Post Image" />
            </div>
